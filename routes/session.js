        ...
        users.validateLogin(username, password, function(err, user){
            if (err) {
                if (err.user_not_exists) {
                    return res.render('login', { 'username': username, 'password': '', 'error': 'The user does not exist' });
                }
                else if (err.invalid_password) {
                    return res.render('login', { 'username': username, 'password': '', 'error': 'Invalid password' });
                }
                else {
                    return next(err);
                }
